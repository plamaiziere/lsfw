...
public abstract class PxEquipment extends GenericEquipment {
    ...
    protected void crossRefIpSet(PxNetworkIpSet ipSet, CrossRefContext refContext) {
        ...
        for (PxNetworkObject po : ipSet.getBaseObjects().values()) {
            ...
            switch (po.getType()) {
                case IPRANGE:
                    ip = ((PxNetworkIp) po).getIpRange();
                    break;
                case IPALIAS:
                    ip = ((PxNetworkAlias) po).getIpRange().getIpRange();
                    break;
                default:
                    ip = null;
            }
    ...
    protected MatchResult ruleFilter(IfaceLink link, Direction direction, Probe probe, PxRule rule) {
        ...
        switch (direction) {
            case IN:
                if (rule.getDirection() != PxRuleDirection.IN) return MatchResult.NOT;
                break;
            case OUT:
                if (rule.getDirection() != PxRuleDirection.OUT) return MatchResult.NOT;
                break;
            default:
